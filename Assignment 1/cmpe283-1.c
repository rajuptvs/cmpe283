...
#define IA32_VMX_PROCBASED_CTLS3 0
 ...
 */
void report_capability(struct capability_info *cap, uint8_t len, uint32_t lo,
											 uint32_t hi)
{
	uint8_t i;
...
void detect_vmx_features(void)
{
	...
	pr_info("Procbased Controls Tertiary MSR: 0x%llx\n",
					(uint64_t)(lo | (uint64_t)hi << 32));	
	report_capability(procbasedcontrols3, 4, lo, hi);
}
